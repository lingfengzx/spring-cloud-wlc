...
$(function(){
	...
	$(document).on("click", "#add_notebook", function() {
		$('#can').load('./templates/alert/alert_notebook.html', function(){
	...
	$(document).on("click", "#pc_part_6 .btn_like", function() {
		$('#can').load('./alert/alert_like.html', function(){
			...
			{
				var shareId=$("#sixth_side_right .checked").parent().data("share").cn_share_id;
				var favoriteId=$("#like_button").data("notebook").cn_notebook_id;
				likeShareNote(shareId,favoriteId);
			});
		});
		$('.opacity_bg').show();
    }),
	...
	$(document).on("click", "#pc_part_8 li", function() 
	{
		$(this).siblings('li').children('a').removeClass('checked');
		$(this).children('a').addClass('checked');
		getActivityNoteDetail($(this).data("note"));
    }),
	...
	$(document).on("click", "#modalBasic_15 .btn.btn-primary.sure", function()
	{
		...
		var noteId=$('#select_note li a.checked').parent().data("note").cn_note_id;
		$('.close,.cancle').trigger('click');
		createNoteActivity(noteId);
    }),
	...
	{
		var note=$(this).parent().parent().data("note");
		up(note);
    }),
